// profile-detail.js
// Post detail modal for profile feed, copied and adjusted from home-detail.js and nest-detail.js
...
function renderProfilePostDetail(post, comments) {
    ...
    <div class="modal fade" id="postDetailModal" tabindex="-1" aria-labelledby="postDetailModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        ...
        post.nest_name ?? "Unknown"
    }</a>
                </h5>
                <div style="font-size:1em;color:#888;">u/${
          ...
          <div class="modal-body">
            ...
            <div class="d-flex align-items-center bg-light" style="gap:12px;padding:8px 16px;border-radius:10px;width:max-content;margin-bottom:16px;">
                ...
                <span style="font-weight:600;" id="votes-count-modal">${
                    post.votes_count
                }</span>
                <button class="btn d-flex align-items-center vote-btn" data-vote="-1" data-post="${
                    post.id
    ...
    if (modalEl) {
        ...
        modalEl.addEventListener("click", function (e) {
            ...
            if (voteBtn) {
                ...
                const upvoteBtn = modalEl.querySelector(
                    '.vote-btn[data-vote="1"] .icon-32'
                );
                const downvoteBtn = modalEl.querySelector(
                    '.vote-btn[data-vote="-1"] .icon-32'
                );
                let currentVote = 0;
                ...
                if (
                    downvoteBtn &&
                    downvoteBtn.classList.contains("text-danger")
                )
                    currentVote = -1;
                ...
                fetch(`/posts/${postId}/vote`, {
                    ...
                    .then((data) => {
                        const voteCountSpan =
                            modalEl.querySelector("#votes-count-modal");
                        if (voteCountSpan)
        ...
        if (commentForm) {
            ...
            function submitComment() {
                ...
                fetch(`/posts/${postId}/comment`, {
                    ...
                    .then((data) => {
                        if (!data) return;
                        const commentsList = modalEl.querySelector(
                            "#comments-list-modal"
                        );
...
// Usage: window.showProfilePostDetail(postId)
window.showProfilePostDetail = function (postId) {
    // Find nest_name for the post
    let nestName = null;
    const card = document
        .querySelector(`.card .vote-btn[data-post='${postId}']`)
        ?.closest(".card");
    if (card) {
    ...
    fetch(`/nests/${nestName}/comments/${postId}`)
        ...
        .then((data) => {
            if (!data.post) return;
            showProfileModal(
                renderProfilePostDetail(data.post, data.comments || [])
            );
        });
};
